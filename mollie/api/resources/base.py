...
class ResourceBase:
    ...
    def perform_api_call(
        ...
        resp = self.client.perform_http_call(
            http_method, path, data, params, idempotency_key
        )
        ...
        if resp.status_code < 200 or resp.status_code > 299:
            if "status" in result and (
                result["status"] < 200 or result["status"] > 299
            ):
    ...
    def validate_resource_id(
        cls, resource_id: str, name: str = "Identifier", message: str = ""
    ) -> None:
        """Generic identifier validation."""
        if not message:
            message = f"Invalid {name} '{resource_id}', it should start with '{cls.RESOURCE_ID_PREFIX}'."

        if not resource_id or not str(resource_id).startswith(
            cls.RESOURCE_ID_PREFIX
        ):
...
class ResourceGetMixin(ResourceBase):
    ...
    def from_url(
        ...
        """Utility method to return an object from a full URL (such as from
        _links).

        This method always does a GET request and returns a single
        Object.
        """

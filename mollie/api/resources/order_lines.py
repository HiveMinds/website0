...
class OrderLines(ResourceBase):
    ...
    This class provides various generic methods such as .delete(),
    .update() and .list(), but since the API interface for these methods
    is completely different from other endpoints, all methods don't use
    the generic methods but have custom implementations in this class.
    """
    ...
    def delete_lines(
        self, data: Optional[Dict[str, Any]] = None, **params: Any
    ) -> dict:
        ...
        return self.perform_api_call(
            self.REST_DELETE, path, data=data, params=params
        )

    def delete(self, order_line_id: str, **params: Any) -> dict:
        """Cancel a single orderline.

        This method provides the regular .delete() interface on top of
        .delete_lines().
        """
    ...
    def update(
        ...
        **params: Any,
        ...
        We are manipulating an orderline here, but the API returns the
        full order payload. To be more consistent, we return the updated
        orderline object in stead.

        If you wish to retrieve the order object, you can do so by using
        the order_id property of the orderline.
        """
        resource_path = self.get_resource_path()
        path = f"{resource_path}/{order_line_id}"
        result = self.perform_api_call(
            self.REST_UPDATE, path, data=data, params=params
        )
        ...
        raise DataConsistencyError(
            f"OrderLine with id '{order_line_id}' not found in response."
        )

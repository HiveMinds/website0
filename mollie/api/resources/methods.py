...
class Methods(MethodsBase, ResourceGetMixin, ResourceListMixin):
    ...
    def all(self, **params: Any) -> PaginationList:
        """List all mollie payment methods, including methods that aren't
        activated in your profile."""
...
class ProfileMethods(MethodsBase):
    ...
    def enable(self, method_id: str, **params: Any) -> Method:
        ...
        For vouchers and giftcards, you need to enable specific issuers
        using enable_issuer().
        ...
        if method_id in self.PAYMENT_METHODS_REQUIRING_ISSUER:
            raise IdentifierError(
                f"Cannot enable '{method_id}' method, it requires enabling specific issuers."
            )
    ...
    def disable(self, method_id: str, **params: Any) -> Method:
        ...
        if method_id in self.PAYMENT_METHODS_REQUIRING_ISSUER:
            raise IdentifierError(
                f"Cannot disable '{method_id}' method, it requires disabling specific issuers."
            )
    ...
    def enable_issuer(
        ...
        **params: Any,
        ...
        Vouchers and giftcards need to enable specific issuers, in stead
        of enabling the payment method in general.
        ...
        if method_id not in self.PAYMENT_METHODS_REQUIRING_ISSUER:
            raise IdentifierError(
                f"Payment method '{method_id}' does not support issuers."
            )
    ...
    def disable_issuer(
        self, method_id: str, issuer_id: str, **params: Any
    ) -> Issuer:
        """Disable an issuer for a payment method.

        Vouchers and giftcards need to disable specific issuers, in
        stead of disabling the payment method in general.

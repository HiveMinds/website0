...
class Payments(
    ...
    def delete(
        self, resource_id: str, idempotency_key: str = "", **params: Any
    ) -> dict:
        """Cancel payment and return the payment object.

        Deleting a payment causes the payment status to change to
        canceled. The updated payment object is returned.
        """
        self.validate_resource_id(resource_id, "payment ID")
...
class OrderPayments(PaymentsBase, ResourceCreateMixin):
    ...
    def list(self) -> PaginationList:
        """List the payments that might have been embedded in the related
        order.

        When you receive an EmbedError, you need to embed the payments
        in the parent order.
        """
...
class CustomerPayments(PaymentsBase, ResourceCreateMixin, ResourceListMixin):
    """Resource handler for the `/customers/:customer_id:/payments`
    endpoint."""
...
class SubscriptionPayments(PaymentsBase, ResourceListMixin):
    """Resource handler for the
    `/customers/:customer_id:/subscriptions/:subscription_id:/payments`
    endpoint."""
    ...
    def __init__(
        self, client: "Client", customer: Customer, subscription: Subscription
    ) -> None:
        self._customer = customer
...
class SettlementPayments(PaymentsBase, ResourceListMixin):
    """Resource handler for the `/settlements/:settlement_id:/payments`
    endpoint."""

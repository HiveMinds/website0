...
When a minor version release will be done, remove all code that triggers
the DeprecationWarning subclass. Then rename the DeprecationWarning
subclass to the next minor version (RemovedIn24Warning =>
RemovedIn25Warning). and rename the PendingDeprecation subclass below
also.

This will make all existing PendingDeprecationWarnings change to
DeprecationWarnings.

This deprecation policy is similar to what is used in Django and Django-
rest-framework. See
...
class IdentifierError(RequestSetupError):
    """Errors related to invalid resource identifiers that will be requested
    from the API."""
...
class ResponseHandlingError(Error):
    ...
    When receiving an error while performing a POST, PATCH or DELETE
    action, an idempotency_key is available that can be used for
...
class ResponseError(Error):
    ...
    def __reduce__(self):
        ...
        Exceptions have a custom __reduce__() routine, but that doesn't
        work since we override the __init__ method here.
        """
...
class UnauthorizedError(ResponseError):
    """Your request wasn't executed due to failed authentication.
...
class UnprocessableEntityError(ResponseError):
    """We could not process your request due to another reason than the ones
    listed above.

    The response usually contains a field property to indicate which
...
class EmbedNotFound(Error):
    """We could not access requested data with an object because the required
    embed was missing.

    Some data within an object is only available after it is explicitly
    requested using an embed query parameter. You tried to access data
    which is available from an embed, but the data is missing:
    apparently you forgot to set the related embed parameter.
    """
...
class RemovedIn27Warning(DeprecationWarning):
    """Deprecation warning for features that will be removed in version
    2.7.0."""


class RemovedIn28Warning(PendingDeprecationWarning):
    """Pending deprecation warning for features that will be removed in version
    2.8.0."""
...
class APIDeprecationWarning(DeprecationWarning):
    ...
    This library cannot control when an API feature will be removed. We
    can only label it as 'deprecated' using this warning, and hope that
    library users stop using the feature before the removal.
    """

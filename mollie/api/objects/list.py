...
class ListBase(ObjectBase, ABC):
    ...
    def __next__(self):
        ...
        if self.current is None:
            ...
            self.current += 1
        try:
            item = self["_embedded"][self.object_type.get_object_name()][
                self.current
            ]
    ...
    def __getitem__(self, key):
        ...
        if isinstance(key, slice):
            ...
            _step = key.step or 1
            sliced_data = [
                self["_embedded"][object_name][x]
                for x in range(_start, _stop, _step)
            ]
    ...
    def has_previous(self):
        """Return True if the ObjectList contains an url for the previous
        set."""
...
class PaginationList(ListBase):
    ...
    def __init__(self, result: Any, parent: "ResourceBase", client: "Client"):
        ...
        if result == {}:
            result = {
                "_embedded": {
                    f"{self._parent.object_type.get_object_name()}": []
                },
                "count": 0,
            }
...
class ObjectList(ListBase):
    ...
    It works to similar to PaginationList, but has no pagination (as all
    ...
    def __init__(
        ...
        if result == {}:
            result = {
                "_embedded": {f"{self._object_type.get_object_name()}": []},
                "count": 0,
            }
